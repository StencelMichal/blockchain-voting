...
def generateSignature(message, publicKeys, privateKey, identity):
    ...
    tag = pow(h, privateKey, groupParameters.prime)

    prefix = L + str(tag) + message
    suffix = str(a) + str(useful_methods.power([h, tag], [r, b], groupParameters.prime))
    ...
    return LinkableSignature(tag, s, c)
